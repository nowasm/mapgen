import type { DungeonParameters, NumericRange } from "@mapgen/layout-schema";
...
type RangeKey = Exclude<keyof DungeonParameters, "width" | "height" | "mode" | "layoutWeights">;
const FIELDS: readonly [RangeKey, string, number, number, number, "空间" | "拓扑"][] = [
...
export function ParameterPanel({ controls, stale, onChange, onReset, onGenerate }: Props) {
  ...
  const fields = (group: "空间" | "拓扑") => FIELDS.filter((field) => field[5] === group).map(([key, label, min, max, step]) => (
