...
export function CandidateMap({ index, layout, selected, onSelect }: CandidateMapProps) {
  ...
  return (
    <button
      ...
      type="button"
      aria-label={`候选地图 ${index + 1}，Seed ${layout.seed}`}
      aria-pressed={selected}
      onClick={onSelect}
    >
      <span className="candidate__number">{String(index + 1).padStart(2, "0")}</span>
      <svg viewBox={`0 0 ${layout.grid.width} ${layout.grid.height}`} aria-hidden="true">
        <rect width={layout.grid.width} height={layout.grid.height} className="candidate__paper" />
      ...
      <span className="candidate__seed">{layout.seed}</span>
    </button>
  );
}
